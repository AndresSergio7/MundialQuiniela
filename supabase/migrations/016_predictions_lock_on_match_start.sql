-- Replace deadline-based prediction lock with match-status-based lock.
-- Rule: predictions are editable while every match is 'scheduled' or 'postponed'.
-- As soon as any match becomes 'live' or 'finished', all edits are blocked.

-- INSERT: replace deadline check with match-status check
...
-- UPDATE: same replacement (016 supersedes 014 for the UPDATE policy)
...
-- Update submit_quiniela RPC to use the same lock rule.
...
BEGIN
  ...
  -- Block submit once any match has started
  ...
  IF p_is_valid THEN
    UPDATE predictions
       SET is_locked = true, updated_at = v_now
...
-- Compatibility overload (parameter order variant)
